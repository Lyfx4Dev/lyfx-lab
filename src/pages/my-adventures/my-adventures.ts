...
export class MyAdventuresPage {
  ...
  getMyAdventures() {
    this.firebaseService.getOrders(this.user.uid)
      .then((querySnapshot) => {
        let array = [];
        ...
        if(array.length != 0){
          //Get adventures info
          this.firebaseService.getOrdersWithAdventures(this.user.uid)
          .subscribe((res) => {
            this.adventures = res;
  ...
  ionViewDidLoad() {
    //Analytics log
    this.firebaseService.eventPageView('Calendar page');
 }
